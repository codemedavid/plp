...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      ...
      setInfo(`Account created! Please check your Gmail inbox at ${trimmedEmail} for a confirmation email. Don't forget to check your Spam or Promotions folder if you don't see it.`);
    } else {
      onClose();
      setEmail('');
      setPassword('');
      ...
      <div ref={dialogRef} className="relative w-full max-w-md bg-white rounded-lg shadow-2xl">
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            {info && (
              <div className="flex items-start gap-2 text-xs text-green-800 bg-green-50 border border-green-200 rounded px-3 py-3 leading-relaxed">
                <svg className="w-4 h-4 mt-0.5 shrink-0 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
