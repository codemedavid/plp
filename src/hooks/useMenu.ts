  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('products')
        .select('id, name, description, category, base_price, discount_price, discount_start_date, discount_end_date, discount_active, purity_percentage, molecular_weight, cas_number, sequence, storage_conditions, inclusions, stock_quantity, available, featured, image_url, safety_sheet_url, coa_url, sort_order, paired_product_ids, bundle_tiers, created_at, updated_at')
        .eq('available', true)
